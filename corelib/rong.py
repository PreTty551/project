...
class ApiClient(object):
    ...
    def call_api(self, action, params=None, sms_api_host=None, **kwargs):
        ...
        :return:
        """
        _sms_api_host = self.api_host
        if sms_api_host:
            _sms_api_host = sms_api_host
        return self._http_call(
            url = self._join_url(_sms_api_host, "%s.%s" % (action, self.response_type)),
            method="POST",
            data=params,
    ...
    def user_get_token(self, user_id, name, portrait_uri, timeout=3):
